...
export default function TeacherBookingRequests() {
  ...
  const loadPendingBookings = useCallback(async () => {
    ...
    try {
      ...
      const filtered = (response.data || []).filter(b => b.bookingStatus === 'PENDING');
  ...
  const handleUpdateBookingStatus = async (bookingId, newStatus, actionVerb = "update") => {
    // ... (Logika handleUpdateBookingStatus tetap sama seperti di TeacherBookings.jsx Anda sebelumnya)
    // ... (Pastikan memanggil loadPendingBookings() setelah sukses)
    ...
    <div className="container mx-auto p-4 md:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        {pendingBookings.map(booking => (
          // Tampilkan setiap booking request dengan opsi Confirm / Cancel
          // Mirip dengan tampilan kartu di TeacherBookings.jsx sebelumnya,
          // tapi fokus pada aksi Confirm/Cancel untuk status PENDING.
